...
export const Home = () => {
  ...
  let [managerChartsloader, setManagerChartsLoader] = useState(true);
  let [loader, setLoader] = useState(true);
  useEffect(() => {
    dispatch(locationAction.getAllLocations());
    dispatch(chartsAction.managerCharts(setManagerChartsLoader));
    dispatch(chartsAction.managerPatientCharts(setLoader));
  }, [dispatch]);
  ...
  const graphByDate = (arr) => {
    if (arr) {
      ...
      setManagerChartsLoader(true);
      setLoader(true);
      dispatch(
        chartsAction.getManagerChartsByDate(
          data,
          setManagerChartsLoader,
          setLoader
        )
      );
    }
  ...
  let doughnutState = {
    ...
    // charts?.firstChart?.map((val) => val.noOfPatient),
    chartOptions: {
      labels: charts[0].firstChart.map((val) => val._id),
      // charts?.firstChart?.map((val) => val._id),
    },
  };

  let multiBarChart = {
    series: ["labTechnicican", "medicalProfession"].map((val) => {
      // console.log(charts[0]?.secondChart?.map((no) => no[val]));
      return {
        name: val,
        data: charts[0]?.secondChart?.map((no) => no[val]),

        // data: charts[0]?.secondChart?.map((no) => no[val]),
        // charts?.secondChart?.map((no) => no[val]),
      };
    }),
    ...
    md:pr-7 2xl:pr-12 2xl:pl-10  mt-3"
      >
        <div className="manager-panel-title-div">
          <h1 className="panel-title xs:text-center">
            Manager&apos;s Dashboard
          </h1>
        </div>
          ...
          <div className=" xs:flex-col xs:items-center xs:justify-center md:flex-row flex flex-wrap justify-around ">
            ...
            <div className="chart-large-div xs:w-full mb-10  mt-7 ">
              <Card className="cards">
                <div>
                  <div>
                    <p className="cards-title-number">40,504</p>
